...
class CameraView:
    ...
    def mostrar_camaras(self):
        ...
        self.updater = AsyncTreeviewUpdater(self.root, self.tree_camaras, self.estado_camaras_var)
    ...
    def actualizar_estado_camaras(self):
        ...
        try:
            ...
            for serie, info in camaras_activas.items():
                ...
                modo = dispositivo_db.get('modo')
                monitoreando = info.get('monitoreando', False)
                
                if modo == "Detección Movimiento" and not monitoreando:
                     self.camara_manager.iniciar_monitoreo_movimiento(serie, intervalo=2, callback_evento=self.registrar_evento_movimiento)
                     info['monitoreando'] = True 
                elif modo != "Detección Movimiento" and monitoreando:
                     self.camara_manager.detener_monitoreo_movimiento(serie)
                     info['monitoreando'] = False
                ...
                self.tree_camaras.insert("", "end", iid=serie, values=(
                    serie,
                    nombre,
                    ubicacion,
                    info.get('ip', 'N/A'),
            ...
            def check_connection(serie, info):
                return self.camara_manager.tomar_fotografia(serie) is not None

            def get_db_info(serie):
                return self.logic.obtener_dispositivo_por_serie(serie)

            def format_status(total, connected, monitoring):
                return f"Cámaras: {connected}/{total} conectadas - Monitoreo: {monitoring} activos"
    ...
    def iniciar_monitoreo_movimiento(self):
        if not self.camara_manager.camaras_activas:
            ...
            return
        iniciadas = self.camera_ctrl.start_monitoring_all(intervalo=2, callback_evento=self.registrar_evento_movimiento)
        self.actualizar_estado_camaras()
